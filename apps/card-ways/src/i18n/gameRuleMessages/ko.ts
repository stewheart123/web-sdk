export default {
	...
	'GR.MODIFIER_FREE_SPINS.TITLE': '수정자 릴 (프리 스핀)',
	'GR.MODIFIER_FREE_SPINS.TEXT':
		'프리 스핀 중 수정자 릴은 매 스핀마다 활성입니다. 공개된 배수는 기능이 끝날 때까지 유지됩니다. 프리 스핀 중 모든 당첨에 활성 수정자 값이 곱해집니다. 프리 스핀 종료 시 수정자는 ×1로 초기화됩니다.',
	'GR.MAX_WIN.TITLE': '최대 당첨',
	'GR.MAX_WIN.TEXT': '모든 베팅 모드에서 최대 당첨은 기본 베팅의 {maxWin}×입니다. 도달 시 라운드가 종료되고 상한이 지급됩니다.',
	'GR.SECTION.PAYTABLE': '페이테이블',
	'GR.SECTION.WIN_WAYS': '당첨 웨이',
	'GR.WIN_WAYS.TEXT':
		'모든 지급 심볼은 가장 왼쪽 릴부터 인접 릴에서 왼쪽에서 오른쪽으로 지급됩니다. 연속 릴의 심볼만 유효한 웨이를 형성합니다. 스캐터에는 적용되지 않습니다. 비당첨 심볼은 지급되지 않으며 당첨 웨이를 형성할 수 없습니다. 동일 심볼의 여러 웨이는 합산됩니다. 와일드는 웨이 완성을 위해 대체합니다. 심볼이 여러 웨이에서 당첨되면 기여하는 모든 심볼이 하나의 애니메이션으로 강조 표시됩니다.',
	...
	'GR.WIN_WAYS.TEXT.SOCIAL':
		'모든 당첨 심볼은 가장 왼쪽 릴부터 인접 릴에서 왼쪽에서 오른쪽으로 승리합니다. 연속된 릴의 심볼만 유효한 웨이를 형성합니다. 스캐터 심볼에는 적용되지 않습니다. 비당첨 심볼은 승리하지 않으며 당첨 웨이를 형성할 수 없습니다. 동일 심볼의 여러 웨이는 합산됩니다. 와일드 심볼은 웨이 완성을 위해 대체됩니다. 심볼이 여러 웨이에서 당첨되면 기여하는 모든 심볼이 하나의 애니메이션으로 강조 표시됩니다.',
