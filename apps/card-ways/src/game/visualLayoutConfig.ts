...
export type HowToPlayPanelLayoutSettings = {
	...
	y: number;
	/**
	 * Compensates for baked-in card shadow (extends right/bottom on the BG asset).
	 * Panels shift by +bias; FG/text shift by -bias so content sits on the card face.
	 */
	shadowBiasX: number;
	shadowBiasY: number;
...
export const VISUAL_LAYOUT = {
	...
	win: {
		...
		layout: {
			...
			/** Slot-local offset of ways subtitle relative to the amount text center. */
			bigWinWaysOffset: { x: 300, y: 420 },
			/** Absolute font size for the Cinzel ways subtitle on big wins (scaled by layout type). */
			bigWinWaysFontSize: 180,
...
export const SCENE_LABELS = {
	...
	win: {
		...
		bigWays: VISUAL_LAYOUT.win.bigWays.label,
